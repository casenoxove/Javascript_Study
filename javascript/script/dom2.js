...
console.log(header);
/* tag, class 등과 같이 다수가 존재할 수 있는 대상을 DOM으로 선택하면
특정 대상 1개만 잡히는 게 아닌 Collection 기준으로
여러개가 잡히기 때문에 원하는 대상 사용시
정확히 그 대상을 인덱스로 선택해야 한다. */
...
/* 
    ...
    변수 사용 시 인덱스를 붙이지 않고 변수명만 간편하게
    쓸 수 있다. 대신, 인덱스가 고정되므로 다른
    인덱스 번호에 해당하는 태그나 클래스를 사용하고
    싶을 때 변수를 새로 만들어야 한다.
    console.log(aTag2);
    console.log(aTag2)[1];
*/
...
/* querySelector는 tag.class 를 선택 시 메서드 괄호와
문자열 대상 기준으로 가장 먼저 인식된 대상을 출력해준다. */
const aQ  = document.querySelector('a')
console.log(aQ)
const logoQ = document.querySelector('.logo')
// 명령어에 id가 없으니까 앞에 # 붙여줘야함. 클래스는 .
console.log(logoQ)
...
// 버튼 클릭 시 아래 함수 호출하기
function btn_func1(){
    // 위 변수에 대한 데이터 타입 출력하기 (콘솔 한 줄씩 출력)
    ...
    console.log(typeof Number(input_num))
...
function btn_func3(){
    ...
    //'사용자 요청사항은 OOOO 입니다.' 
    input_text.value = window.alert(`사용자 요청사항은 ${input_text.value} 입니다.`);
}
function btn_func4(){
    //사용자 요청사항과 주문수량을 확인하고
    //주문 버튼을 클릭하면 다음과 같이 경고창 출력하기
    //"요청사항은 OOOO, 주문수량은 O개, 총 결제 금액은 OOOO원입니다."
    //제품 1개 당 가격은 1000원
    price = 1000
    let request = input_text.value
    let order = input_num.value
    input_text.value = window.alert(`요청사항은 ${request}, 주문수량은 ${order}개, 총 결제 금액은 ${order * price}원 입니다.`);
}
// 객체.메서드(실행값)
// 객체.속성 = 속성에 대입할 값 (속성에 대입할 값을 오른쪽에 작성)
/* 
    <input type="text" 속성>
    1. name x
    2. value
